<template>
	<view class="infobox">
		<scroll-view scroll-y="true" class="scroll-Y" @scrolltolower="handleReachBottom">
					<view class="biaoti acea-row row-center">历史素材库</view>
					<view class="list m_top_30 acea-row" v-if="total">	
						<view class="img_kuang" @click="onimg($tool.imgReplace(item.path))" v-for="(item,index) in dataList" :key="index">
							<image :src="item?.path ? $tool.imgReplace(item.path) : '/static/img/300-200.png'" class="img" mode="aspectFill"></image>
						</view>
					</view>
...
<script setup>
	...
	const getLsit = () => {
		...
		$http.get('Index/getMaterial', query.value).then(res => {
			if(res.code == 200){
